...
import { Alert, Box, Button, ButtonGroup, Card, Divider, Fade, Grid, Typography, Slide, Collapse } from "@material-ui/core";
import { AppContext, Theme } from "../index";
import axios from "axios";
import { BsCalendar4Event, BsCalendar4Range } from 'react-icons/bs';
import { AiOutlineMinus, AiOutlinePlus, AiOutlineTeam, AiOutlineUser } from 'react-icons/ai';
...
const Filter = props => {
    ...
    const
        ...
        [adults, setAdults] = useState(info.filters.guests.adults ? info.filters.guests.adults : 1),
        [children, setChildren] = useState(info.filters.guests.children ? info.filters.guests.children : 0),

        [startDate, setStartDate] = useState(info.filters.reservationDates.start ? info.filters.reservationDates.start : new Date()),
        [startDatePickerOpen, setStartDatePickerOpen] = useState(false),
        [endDate, setEndDate] = useState(info.filters.reservationDates.end ? info.filters.reservationDates.end : new Date(startDate.getTime() + (24 * 60 * 60 * 1000))),
        ...
        filterApply = data => {
            let rooms = info.filters.reservationDates.start === startDate && info.filters.reservationDates.end === endDate ? {
                ...info,
            ...
            info.roomSelection.totalPayment = 0;
        ...
        reserveDate = data => props.page === "intro" ? setReserveDate(data) : filterApply(data),
        ...
        <>
            <Box sx={{
                ...
                <Fade in={props.filterOpen}>
                    <Box onClick={() => props.setFilterOpen(false)} sx={{
                    ...
                    <Card sx={{
                            ...
                            <Grid container spacing={3}>
                                <Grid item xs={12} sx={errorStyle(noRoom)}>
                                    <Collapse in={noRoom} timeout={{ enter: 500, exit: 500 }}>
                                        <Box>
                                            <Alert severity="error" >There are no rooms available on the dates you've chosen — let's go check another date!</Alert>
                                        </Box>
                                    </Collapse>
                                </Grid>
                                <Grid item xs={12} sx={errorStyle(bookingError)}>
                                    <Collapse in={bookingError} timeout={{ enter: 500, exit: 500 }}>
                                        <Box>
                                    ...
                                    <Card sx={{
                                        ...
                                        <Box px={3}>
                                            <LocalizationProvider dateAdapter={DateFnsUtils}>
                                                <MobileDatePicker showToolbar={false} sx={{ fontFamily: Theme.typography.fontFamily.sansSerif }}
                                                    minDate={new Date(new Date().getTime() + (24 * 60 * 60 * 1000))} onChange={setStartDate}
                                                    onClose={() => (
                                                    ...
                                                    renderInput={({
                                                        ...
                                                        <div ref={ref} {...other}>
                                                            <input disabled={disabled} onChange={onChange}
                                                                style={{ display: "none" }} value={value}
                                                                {...inputProps} />
                                                            <Button onClick={() =>
                                            ...
                                            <Divider />
                                            <LocalizationProvider dateAdapter={DateFnsUtils}>
                                                <MobileDatePicker showToolbar={false} sx={{ fontFamily: Theme.typography.fontFamily.sansSerif }} minDate={new Date(startDate.getTime() + (24 * 60 * 60 * 1000))} onChange={setEndDate}
                                                    onClose={() =>
                                                        setEndDatePickerOpen(false)}
                                                    open={endDatePickerOpen}
                                                    okText={"Confirm End Date"}
                                                    renderInput={({
                                                        ref,
                                                        inputProps,
                                                        disabled,
                                                        ...
                                                        <div
                                                            ...
                                                            <input disabled={disabled} onChange={onChange}
                                                                ...
                                                                {...inputProps}
                                                            />
                                                            <Button sx={Theme.typography.filterText}
                                                                onClick={() =>
                                        ...
                                        <Box px={3}>
                                            ...
                                            <Box py={2} sx={{
                                                ...
                                                <ButtonGroup variant="contained">
                                                    <Button disabled={adults === defaults.adults.min ? true : false}
                                                        sx={{ backgroundColor: Theme.palette.background.light }}
                                                        onClick={() => adults === defaults.adults.min ? '' : setAdults(adults => adults - 1)}>
                                                        <AiOutlineMinus
                                                            size={iconSize}
                                                        />
                                                    </Button>
                                            ...
                                            <Box py={2} sx={{
                                                ...
                                                <ButtonGroup variant="contained">
                                                    ...
                                                    <Button sx={{
                                                        ...
                                                        disabled={
                                                            children === defaults.children.max ?
                                                                true :
                                                                false
                                                        }
                                                        onClick={() =>
                        ...
                        <Box mb={2} mx={2} sx={{
                            ...
                            <Box>
                                ...
                                <Button onClick={() => props.setFilterOpen(false)} sx={{
                                    borderRadius: {
                                        xs: Theme.shape.borderRadius,
                                        sm: Theme.shape.borderRadius,
                                    },
